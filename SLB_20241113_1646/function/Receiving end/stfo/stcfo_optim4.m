function [sto_est,cfo_est,u1,cp_num] = stcfo_optim4(recv,sys_Parameter,t_off)
...
sto_scan_sts = 1:(max_scane_sto0-256);
...
for i = 1:20
    for j = sto_scan_sts
        ...
        recv_slots_seg = reshape(recv_slots,[seg_num_sts,256/seg_num_sts]);
        recv_slots_energy = sum(abs(conj(recv_slots_seg).*recv_slots_seg).^2,2); %能量计算
        recv_slots_energy(recv_slots_energy == 0) = 1; %避免出现nan
        % 相关运算
        cur_fts_ref = reshape(sts_k_all(:,i),[seg_num_sts,256/seg_num_sts]);
...
u2 = u2_maxs(1);
sts_maxs_p = cr_sts_max_peaks(u2,1);
if(abs(sts_maxs_p-18-1-t_off)~=0)
    fprintf("sts err: %+-d\n",abs(sts_maxs_p-18-1-t_off));
end
u1=1;
% %% 先估计时偏和u1
...
%         if(abs(loc1(j)-loc_max1a(1))<=256 || loc1(j)<25)
%             pk1(j) = -1 * pk1(j);
%         end
%     end
...
% if(cr_sto1_maxv(u1,1)>cr_sto1_maxv(u1,2))
...
for i = 1:2
    ...
    for k = 0:2
        for j = 1:cfo_scan_len
            % 当前检测段索引
            seq_order = sts_maxs_p-18+256+18*2+k:sts_maxs_p-18+256+18*2 +256-1+k;
            % 接收信号段
