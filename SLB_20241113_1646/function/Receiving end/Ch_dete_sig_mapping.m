function [tx_FT_block, index] = Ch_st_ref_sig_mapping(tx_FT_block, sys_Parameter, frame_Parameter, index)
    ...
    dection_frame               = index.dection_frame;
    dection_symbol              = index.dection_symbol;

    % 单天线的情况（多天线加个判断条件即可）
    % 索引是否为空判断 
    if isempty(dection_frame)
        % disp('探测信号索引为空，本超帧无信道探测参考信号');
        return; 
    end

    % 遍历每个无线帧
    for n = 1:length(dection_frame)
        current_frame = dection_frame(n);  % 当前无线帧索引
          % 当前无线帧中有内容的符号索引

        % 遍历当前无线帧的每个符号位置
        for l = 1:length(dection_symbol{n})
        symbols_in_frame = dection_symbol{n}(l);
        % 生成伪随机 QPSK 序列
